  ...
  const columns = [
      ...
      render: (replicas, record) => {
        ...
        return (
          <div className="replicas">
            <a onClick={() => props.showDiskReplicaModal(record, node)}>
              {numberOfReplicas}
            </a>
          </div>
        )
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        const total = ((record.storageMaximum - record.storageReserved) * props.storageOverProvisioningPercentage) / 100
        const p = total === 0 ? 0 : Math.round((allocated / total) * 100)
        return (
          <div className="allocated">
            <div>
              <Tooltip title={`${p}%`}>
                <Progress strokeWidth={14} status={getStorageProgressStatus(props.minimalSchedulingQuotaWarning, p)} percent={p > 100 ? 100 : p} showInfo={false} />
              </Tooltip>
            </div>
