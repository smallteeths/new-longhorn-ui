...
import { formatMib, byteToGi, getStorageProgressStatus } from '../../utils/formater'
import { pagination } from '../../utils/page'
import ModalBlur from '../../components/common/modalBlur'
...
class HostList extends React.Component {
  ...
  render() {
        ...
        render: (text, record) => {
          return (
            <div style={{ textAlign: 'center' }}>
              <div>{text} {record && (record.region || record.zone) && <Tooltip
                title={<span> region: {record.region} <br></br> zone: {record.zone} </span>}>
                <EnvironmentOutlined className={classnames(styles.link, {
                  'ml-2': true,
                })}/>
              </Tooltip> } </div>
    ...
    return (
      <div id="hostTable" className='w-full h-full'>
        <Table
        ...
        />
        <ModalBlur width={980} title={'Components'} open={this.props.instanceManagerVisible} onCancel={() => { this.modalBlurCancel() }} onOk={() => { this.modalBlurOk() }} hasOnCancel={true}>
          <InstanceManager defaultInstanceManager={defaultInstanceManager} defaultEngineImage={defaultEngineImage} currentNode={currentNode} />
        </ModalBlur>
      </div>
    )
  }
